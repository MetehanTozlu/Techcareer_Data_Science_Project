...
# ############################################ İş Problemi ############################################ Gezinomi
# yaptığı satışların bazı özelliklerini kullanarak seviye tabanlı (level based) yeni satış tanımları oluşturmak ve bu
# yeni satış tanımlarına göre segmentler oluşturup bu segmentlere göre yeni gelebilecek müşterilerin şirkete ortalama
# ne kadar kazandırabileceğini tahmin etmek istemektedir. Örneğin: Antalya’dan Herşey Dahil bir otele yoğun bir
# dönemde gitmek isteyen bir müşterinin ortalama ne kadar kazandırabileceği belirlenmek isteniyor.
# ############################################ PROJE GÖREVLERİ ############################################
...
agg_df: DataFrame = df.groupby(["SaleCityName", "ConceptName", "Seasons"]).agg({"Price": "mean"}).sort_values("Price")
agg_df.head(20)
...
var = agg_df[agg_df["sales_level_based"] == new_user]
print(var)
